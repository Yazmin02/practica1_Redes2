...
public class Servidor {
        ...
        try {
            ...
            while ((inputLine = in.readLine()) != null) {
                if ("quit".equals(inputLine)) {
                    out.println("Terminando conexión...");
                    break;
                }
                processCommand(inputLine);
            }
        } catch (IOException e) {
            System.out.println("Error en conexión del cliente: " + e.getMessage());
        } finally {
            stop();
        }
    }
    ...
    private void processCommand(String command) {
        ...
        try {
            ...
            response = "No se pudo procesar el comando: " + e.getMessage();
        }
    ...
    private String listDirectory(Path directory) throws IOException {
        try (Stream<Path> paths = Files.list(directory)) {
            List<String> fileList = paths.map(path -> path.getFileName().toString())
                    .collect(Collectors.toList());
            return String.join("\n", fileList); // Devuelve la lista como una única cadena
        }
    }
